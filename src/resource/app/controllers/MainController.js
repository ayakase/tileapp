    ...
    home(req, res, next) {
        try {
            if (req.originalMethod == 'GET') {
                ...
                if (account_verify) {
                    res.render('homepage', { error: false, errorLog: '', User: 'UsernameBug', signin: true});
                } else {
                    res.render('homepage', { error: false, errorLog: '', User: '',  signin: false});
                }
            }
        } catch {
            res.render('homepage', { error: false, errorLog: '', User: '',  signin: false});
            ...
            mysqldb.connect().query(signinQuery(req.body.user_account, req.body.user_password), function (err1, result1, fields1) {
                if(err1) {
                    ...
                    if(result1.length == 1) {
                        ...
                        res.render('homepage' , { error: true, errorLog: '*Sai tài khoản hoặc mật khẩu', User: req.body.user_account, signin: false});
                    }
                }
            })
        }
