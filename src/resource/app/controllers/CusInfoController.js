...
class CusInfo {
    useCusInfo(req, res, next) {
        try {
            if (req.originalMethod == 'GET') {
                var token = req.cookies.token;
                var account_verify = jwt.verify(token, jwtEncode.jwtEncode());
                if (account_verify) {
                    res.render('cusInfo', { error: false, errorLog: '', User: 'UsernameBug', signin: true });
                } else {
                    res.render('cusInfo', { error: false, errorLog: '', User: '',  signin: false });
                }
            }
        } catch {
            res.render('cusInfo', { error: false, errorLog: '', User: '',  signin: false });
