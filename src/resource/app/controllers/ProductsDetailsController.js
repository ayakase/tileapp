...
class ProductsDetails {
    useProductsDetails(req, res, next) {
        try {
            if (req.originalMethod == 'GET') {
                var token = req.cookies.token;
                var account_verify = jwt.verify(token, jwtEncode.jwtEncode());
                if (account_verify) {
                    res.render('products_details', { error: false, errorLog: '', User: 'UsernameBug', signin: true });
                } else {
                    res.render('products_details', { error: false, errorLog: '', User: '',  signin: false });
                }
            }
        } catch {
            res.render('products_details', { error: false, errorLog: '', User: '',  signin: false });
...
module.exports = new ProductsDetails;